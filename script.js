...
document.addEventListener('DOMContentLoaded', function() {
    ...
    rateTypeBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            var extra1 = document.getElementById('rate-type-badge-extra-1');
            var extra2 = document.getElementById('rate-type-badge-extra-2');
            if (badgeText && badgeDesc) {
                if (rateType === 'floating') {
                    badgeText.textContent = 'По подтверждениям';
                    badgeDesc.textContent = 'Курс фиксируется после получения необходимых подтверждений в сети и определяется по фактической средней цене открытия рыночного ордера на бирже Gate.';
                    if (extra1) extra1.style.display = '';
                    if (extra2) extra2.style.display = '';
                } else if (rateType === 'fixed') {
                    badgeText.textContent = 'Фиксированный';
                    badgeDesc.textContent = 'Курс фиксируется в момент создания заявки в пределах 0,5%. Если до момента исполнения заявки курс изменится более чем на 0,5%, обработка заявки будет приостановлена до согласования нового курса с вами.';
